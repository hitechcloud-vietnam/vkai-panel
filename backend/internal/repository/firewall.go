...
func (r *FirewallRepository) GetByID(ctx context.Context, id uuid.UUID) (*models.FirewallRule, error) {
	var rule models.FirewallRule
	query := `SELECT * FROM firewall_rules WHERE id = $1`
...
func (r *FirewallRepository) Update(ctx context.Context, rule *models.FirewallRule) error {
	query := `UPDATE firewall_rules SET protocol = $2, port = $3, source = $4, action = $5, direction = $6, status = $7, updated_at = NOW() WHERE id = $1`
	_, err := r.db.ExecContext(ctx, query,
		rule.ID, rule.Protocol, rule.Port, rule.Source, rule.Action, rule.Direction, rule.Status,
	)
	return err
}

func (r *FirewallRepository) Delete(ctx context.Context, id uuid.UUID) error {
