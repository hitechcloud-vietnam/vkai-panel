...
func (r *DatabaseRepository) GetServerByID(ctx context.Context, id uuid.UUID) (*models.DatabaseServer, error) {
	var s models.DatabaseServer
	query := `SELECT * FROM database_servers WHERE id = $1`
...
func (r *DatabaseRepository) DeleteServer(ctx context.Context, id uuid.UUID) error {
	query := `DELETE FROM database_servers WHERE id = $1`
...
func (r *DatabaseRepository) GetEntryByID(ctx context.Context, id uuid.UUID) (*models.DatabaseEntry, error) {
	var e models.DatabaseEntry
	query := `SELECT * FROM database_entries WHERE id = $1`
...
func (r *DatabaseRepository) ListEntriesByServer(ctx context.Context, serverID uuid.UUID) ([]models.DatabaseEntry, error) {
	var entries []models.DatabaseEntry
	query := `SELECT * FROM database_entries WHERE database_server_id = $1 ORDER BY name`
	if err := r.db.SelectContext(ctx, &entries, query, serverID); err != nil {
		return nil, err
	}
	return entries, nil
}
...
func (r *DatabaseRepository) DeleteEntry(ctx context.Context, id uuid.UUID) error {
	query := `DELETE FROM database_entries WHERE id = $1`
	_, err := r.db.ExecContext(ctx, query, id)
	return err
}

func (r *DatabaseRepository) UpdateEntryPassword(ctx context.Context, id uuid.UUID, password string) error {
	query := `UPDATE database_entries SET password = $2, updated_at = NOW() WHERE id = $1`
