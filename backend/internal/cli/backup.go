...
func runBackupCreate(cmd *cobra.Command, args []string) {
	backupDir := "/opt/vkai-panel/backups"
...
func createFullBackup(backupPath string) {
	...
	tempDir := "/tmp/vkai-backup-" + time.Now().Format("20060102150405")
	...
	copyFile("/opt/vkai-panel/config.yaml", filepath.Join(configDir, "config.yaml"))
	copyFile("/opt/vkai-panel/.env", filepath.Join(configDir, ".env"))
...
func createConfigBackup(backupPath string) {
	...
	tempDir := "/tmp/vkai-config-backup-" + time.Now().Format("20060102150405")
	os.MkdirAll(tempDir, 0755)
	defer os.RemoveAll(tempDir)

	// Copy config files
	copyFile("/opt/vkai-panel/config.yaml", filepath.Join(tempDir, "config.yaml"))
	copyFile("/opt/vkai-panel/.env", filepath.Join(tempDir, ".env"))
...
func runBackupRestore(cmd *cobra.Command, args []string) {
	...
	tempDir := "/tmp/vkai-restore-" + time.Now().Format("20060102150405")
	os.MkdirAll(tempDir, 0755)
	defer os.RemoveAll(tempDir)
	...
	if _, err := os.Stat(configFile); err == nil {
		...
		copyFile(configFile, "/opt/vkai-panel/config.yaml")
	}

	printSuccess("Backup restored successfully")
	printInfo("Restart services: systemctl restart vkai-panel-api vkai-panel-frontend")
}
