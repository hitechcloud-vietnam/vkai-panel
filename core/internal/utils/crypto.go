...
// DatabaseEncryptionKey returns the 32 byte key used to encrypt secrets (such
// as managed database passwords) before they are written to the panel
// database. The key is read from VKAI_SECRET_KEY as 64 hex characters or as
// base64. There is deliberately no default: storing customer credentials in
// plaintext must fail loudly rather than happen silently.
func DatabaseEncryptionKey() ([]byte, error) {
	raw := strings.TrimSpace(os.Getenv("VKAI_SECRET_KEY"))
	if raw == "" {
		return nil, errors.New("VKAI_SECRET_KEY is not set: cannot store database credentials safely")
	}
