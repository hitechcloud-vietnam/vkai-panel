...
func (h *FirewallHandler) Get(c *gin.Context) {
	...
	rule, err := h.firewallService.GetByID(c.Request.Context(), middleware.GetTenantID(c), id)
	if err != nil {
...
func (h *FirewallHandler) Delete(c *gin.Context) {
	...
	if err := h.firewallService.Delete(c.Request.Context(), middleware.GetTenantID(c), id); err != nil {
		utils.InternalError(c, err.Error())
		return
	}
