...
`
	...
	// Apache will not create the log directory for itself; a missing one is a
...
func (a *ApacheAdapter) ConfigureSSL(ctx context.Context, domain, certPath, keyPath string) error {
    ...
    ErrorLog %s/ssl_error.log
    CustomLog %s/ssl_access.log combined
</VirtualHost>
`, domain, siteRoot, certPath, keyPath, siteRoot, logDir, logDir)
