...
// Binding tags are the first gate on values that later become file paths,
// shell-adjacent arguments or SQL identifiers. The service layer re-validates
// them, but rejecting obvious junk at the edge keeps it out of the logs too.
type CreateWebsiteRequest struct {
	Domain        string    `json:"domain" binding:"required,hostname,max=253"`
	ServerID      uuid.UUID `json:"server_id" binding:"required"`
	RootDir       string    `json:"root_dir" binding:"omitempty,max=512"`
	WebServerType string    `json:"web_server_type" binding:"required,oneof=nginx apache caddy litespeed openlitespeed traefik"`
	PHPVersion    string    `json:"php_version" binding:"omitempty,max=16"`
	SiteType      string    `json:"site_type" binding:"omitempty,max=32"`
}
...
type CreateDBEntryRequest struct {
	...
	// The exact identifier charset is enforced in the service layer by
	// utils.ValidateSQLIdentifier; this is only a length guard.
	Name      string `json:"name" binding:"required,max=63"`
	Username  string `json:"username" binding:"required,max=63"`
	Password  string `json:"password" binding:"required,min=12,max=128"`
	Charset   string `json:"charset" binding:"omitempty,alphanum,max=32"`
	Collation string `json:"collation" binding:"omitempty,max=64"`
}

type CreateCronJobRequest struct {
	ServerID uuid.UUID `json:"server_id" binding:"required"`
	Name     string    `json:"name" binding:"required,max=128"`
	Command  string    `json:"command" binding:"required,max=4096"`
	Schedule string    `json:"schedule" binding:"required,max=128"`
...
type CreateFirewallRuleRequest struct {
	...
	Port      string    `json:"port" binding:"omitempty,max=11"`
	Source    string    `json:"source" binding:"omitempty,max=43"`
	Action    string    `json:"action" binding:"required,oneof=ACCEPT DROP REJECT LOG"`
	Direction string    `json:"direction" binding:"omitempty,oneof=in out"`
}

type CreateBackupJobRequest struct {
	Name        string    `json:"name" binding:"required,max=128"`
	Type        string    `json:"type" binding:"required,oneof=website database files"`
	ResourceID  uuid.UUID `json:"resource_id" binding:"required"`
	Destination string    `json:"destination" binding:"omitempty,max=512"`
	Schedule    string    `json:"schedule" binding:"omitempty,max=128"`
	Retention   int       `json:"retention" binding:"omitempty,min=1,max=3650"`
